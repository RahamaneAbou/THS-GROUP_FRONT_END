  ...
  <div class="note-secretaire">
    ...
    <div class="liste-notes">
      <div class="en-tete-liste">
        <h3>Gestion des Notes ({{ notesFiltrees.length }})</h3>
        <button @click="exporterNotesPDF" class="btn-export">
          <span class="icon">📄</span>
          Exporter PDF
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    // Exporter en PDF
    exporterNotesPDF() {
      const donnees = this.notesFiltrees.map(note => ({
        etudiant: `${note.etudiant.prenom} ${note.etudiant.nom}`,
        cours: note.cours.nom,
        formation: note.cours.formation ? note.cours.formation.nom : 'Non définie',
        note: `${note.valeur}/20`,
        date: this.formaterDate(note.DateAttribution)
      }))
      
      console.log('Export notes PDF:', donnees)
...
<style scoped>
/* Styles similaires à NoteGestion avec adaptations pour le secrétaire */
.note-secretaire {
  max-width: 1400px;
}
...
/* Modal styles - mêmes que les autres composants */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
